...
qaBox.innerHTML='<button id="qa-walk">Movement sweep</button> <button id="qa-fight">Live fight</button> <button id="qa-wound">Wound showcase</button> <button id="qa-pause">Pause</button> <button id="qa-close">Hide diagnostics</button><pre id="qa-info"></pre>';
document.body.appendChild(qaBox);
let qaWalk=false,qaT=0,qaFrames=[],qaLast=performance.now();
const qaIntent=playerIntent,qaSim=simulate;
playerIntent=function(f,e){if(qaWalk)return;return qaIntent(f,e);};
simulate=function(dt){
  if(qaWalk){
    qaT+=dt;const a=qaT*.5;
    player.vel.set(Math.cos(a)*.8,0,Math.sin(a)*.8);
    enemy.vel.set(0,0,0);enemyAI.update=()=>{};
    player.tipTarget.copy(player.pos).addScaledVector(DIRY(player.bodyYaw),1.1).setY(1.3);
  }
  qaSim(dt);
};
document.getElementById('qa-walk').onclick=()=>{
...
document.getElementById('qa-fight').onclick=()=>{qaWalk=false;document.getElementById('menu').classList.add('hidden');restart();};
document.getElementById('qa-wound').onclick=()=>{
  qaWalk=false;document.getElementById('menu').classList.add('hidden');restart();game.introT=0;
...
document.getElementById('qa-close').onclick=()=>qaBox.remove();
